...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, { fontSize, fontFamily, charset, invert }: AsciiFilterOptions = {}) {
    ...
    if (!ctx) {
      throw new Error('Could not obtain 2d context for AsciiFilter canvas');
    }
    this.context = ctx;
...
class CanvasTxt {
  ...
  constructor(txt: string, { fontSize = 200, fontFamily = 'Arial', color = '#fdf9f3' }: CanvasTxtOptions = {}) {
    ...
    const ctx = this.canvas.getContext('2d');
    if (!ctx) {
      throw new Error('Could not obtain 2d context for CanvasTxt canvas');
    }
    this.context = ctx;
    this.txt = txt;
...
class CanvAscii {
  ...
  constructor(
    { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves }: CanvAsciiParams,
